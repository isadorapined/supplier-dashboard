...
> **Build note (dashboard session 2, 25 September 2026).** Lines 1–3 say "unchanged from v1.1", but the portal's v3.1 went live the same day with open magic-link signup, so every verified supplier is now `authenticated`. With the builder's direction ("magic link for the portal, authentication + authorisation for the dashboard"), lines 1–3 are built as a SELECT policy for an **active `user_roles` row** (any role), not bare `authenticated`, and line 4's check reads the same row. The rows each role sees are unchanged: Reviewer, Procurement and Admin read everything; suppliers and deactivated accounts read nothing. See docs/supabase-setup.md → RLS.
